class MyInfoPage {
    ...
    fillEmployeeDetails(EmpIdTest, OtherIdTest, DriversLicenseDate, ssnNumber, sinNumber){
        cy.get(this.genericField).eq(3).clear().type(EmpIdTest)
        cy.get(this.genericField).eq(4).clear().type(OtherIdTest)
        cy.get(this.genericField).eq(5).clear().type(DriversLicenseDate)
        cy.get(this.dateField).eq(0).clear().click().type(ssnNumber)
        cy.get(this.dateField).eq(1).clear().click().type(sinNumber)
    }
    saveForm(){
    ...
    fillStatus(){
        cy.get(this.comboboxButton).eq(0).click()
        cy.get('.oxd-select-dropdown').contains('Brazilian').click()
        cy.get(this.comboboxButton).eq(1).click()
